from iCaRL_9 import iCaRLmodel              #modify
...
for train_no in range(1,4):
    ...
    for i in range(10): #was 10,5
        ...
        filename5 = f'{filenames}/model/task_{i}_training_time={task_end_time - task_start_time:.2f}_train={train_no}.txt'     #modify
        torch.save((task_end_time - task_start_time), filename5)
        # if i==9:
    end_time = time.time()

    # print('Total training time: {:.2f} seconds'.format(end_time - start_time))
    filename6 = f'{filenames}/model/total_training_time= {end_time - start_time:.2f}_train={train_no}.txt'        #modify
    torch.save((end_time - start_time), filename6)
