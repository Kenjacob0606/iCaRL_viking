...
#cifar100_class=20_def

#TRAIN 1

numclass=20#num of classes learned initially, will be updated in incremental learning   
feature_extractor=resnet18_cbam(num_classes=numclass) #try other resnets
img_size=32
batch_size=128  
task_size=20 #num of classes learned each task
memory_size= 2000
epochs=70 #was 100
learning_rate=2.0
file=1
...
for i in range(5): #was 10,5
    ...
    filename = f'cifar100_class=20_def/model/task_{i}_training_time= {task_end_time - task_start_time:.2f}_train={train_no}.txt'
    torch.save((task_end_time - task_start_time), filename)
    # if i==9:
end_time = time.time()

# print('Total training time: {:.2f} seconds'.format(end_time - start_time))
filename2 = f'cifar100_class=20_def/model/total_training_time= {end_time - start_time:.2f}_train={train_no}.txt'
...
for i in range(5): #was 10,5
    ...
    model.afterTrain(accuracy)
    task_end_time = time.time()
    filename = f'cifar100_class=20_def/model/task_{i}_training_time= {task_end_time - task_start_time:.2f}_train={train_no}.txt'  #modify
    torch.save((task_end_time - task_start_time), filename)
    # if i==9:
end_time = time.time()

# print('Total training time: {:.2f} seconds'.format(end_time - start_time))
filename2 = f'cifar100_class=20_def/model/total_training_time= {end_time - start_time:.2f}_train={train_no}.txt'  #modify
torch.save((end_time - start_time), filename2)
