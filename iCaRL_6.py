...
class iCaRLmodel:
    ...
    def afterTrain(self,accuracy):
        ...
        filename = f'cifar100_lr=2.05_def/model/{self.task_num}-accuracy-{accuracy:.3f}_KNN_accuracy-{KNN_accuracy:.3f}_increment-{i + 10}_net_train={self.train_no}.pkl'  # changed : to -
        self.accuracy_list.append(KNN_accuracy)  #newly added
        filename2 = f'cifar100_lr=2.05_def/model/model_class_mean_{self.task_num}_train={self.train_no}.pth'  #newly added
...
#GRAPHS
        if self.task_num==10:                    #Changable param depending on how many class each task
            if self.task_num==10 and self.dataset=='MNIST':     
                ...
                plt.ylabel("Accuracy")
                plt.title("Accuracy vs Tasks")
                plt.savefig(f'cifar100_lr=2.05_def/model/accuracy_vs_tasks_train={self.train_no}.png') 
                # plt.show()
                plt.plot(self.task_list, self.accuracy_list, "g+-")
                plt.xticks(range(len(self.accuracy_list)+1))
                plt.yticks(range(0, 101, 10))
                ...
                plt.ylabel("Accuracy")
                plt.title("Accuracy vs Tasks")
                plt.savefig(f'cifar100_lr=2.05_def/model/accuracy_vs_tasks_line_train={self.train_no}.png') 
                # plt.show()
