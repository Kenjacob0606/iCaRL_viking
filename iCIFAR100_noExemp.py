...
class iCIFAR100(CIFAR100):
    ...
    def concatenate(self, datas, labels):
        con_data = datas[0]                 #add first class data and label
        con_label = labels[0]
        for i in range(1, len(datas)):
            con_data = np.concatenate((con_data, datas[i]), axis=0)     #add subsequent classes data and label
            con_label = np.concatenate((con_label, labels[i]), axis=0)
        return con_data, con_label

    def getTestData(self, classes):
        datas, labels = [], []
        for label in range(classes[0], classes[1]):             #classes set in iCaRL.py. ie,0-10,10-20,20-30...
    ...
    def getTrainData(self, classes, exemplar_set=None):
