...
#cifar100_class=50_def

#TRAIN 1

numclass=50#num of classes learned initially, will be updated in incremental learning
feature_extractor=resnet18_cbam(num_classes=numclass) #try other resnets
img_size=32
batch_size=128  
task_size=50 #num of classes learned each task
memory_size= 2000
epochs=70 #was 100
learning_rate=2.0
file=1
...
for i in range(2): #was 10,5
    ...
    filename = f'cifar100_class=50_def/model/task_{i}_training_time= {task_end_time - task_start_time:.2f}_train={train_no}.txt'
    torch.save((task_end_time - task_start_time), filename)
    # if i==9:
end_time = time.time()

# print('Total training time: {:.2f} seconds'.format(end_time - start_time))
filename2 = f'cifar100_class=50_def/model/total_training_time= {end_time - start_time:.2f}_train={train_no}.txt'
