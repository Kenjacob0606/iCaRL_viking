...
from iMNIST_mul_latest import iMNIST
from myNetwork import network
from iCIFAR10_mul_latest import iCIFAR10
...
class iCaRLmodel:
    ...
    def __init__(self,numclass,feature_extractor,batch_size,task_size,memory_size,epochs,learning_rate,dataset,file, train_no, filenames, dataset_offset=0):
        ...
        self.file = file    #newly added
        self.dataset = dataset  #newly added
        ...
        self.exemplar_classify_transforms = [] #new
        self.class_mean_set = []
    ...
    def load_dataset(self, dataset):
        if dataset == 'CIFAR100':
            ...
            self.train_dataset = iCIFAR10('CIFAR10', transform=self.train_transform, download=True)                       #CIFAR10
            self.test_dataset = iCIFAR10('CIFAR10', test_transform=self.test_transform, train=False, download=True)
            ...
            print(self.train_dataset.transform)
    ...
    def _get_train_and_test_dataloader(self, classes):
        transformed_exemplars = []
        if len(self.exemplar_set) != 0:
            for i,exemplar in enumerate(self.exemplar_set):
                transformed_class = []
                trans = self.exemplar_transforms[i]
                for img in exemplar:
                    if isinstance(img,torch.Tensor):
                        img = img.cpu().numpy()
                    transformed_exemplar = trans(Image.fromarray(img))
                    transformed_class.append(transformed_exemplar)
                transformed_exemplars.append(transformed_class)
        
        self.train_dataset.getTrainData(classes, transformed_exemplars, offset=self.dataset_offset) 
        self.test_dataset.getTestData(classes, offset=self.dataset_offset) 
    ...
    def afterTrain(self,accuracy):
        ...
        for i in range(self.numclass-self.task_size,self.numclass): #loop over newly learned classes only
            ...
            images=self.train_dataset.get_image_class(i, self.dataset_offset) #offset added
        ...
        filename = f'{self.filenames}/model/{self.task_num}-accuracy-{accuracy:.3f}_KNN_accuracy-{KNN_accuracy:.3f}_increment-{i + 10}_net_train={self.train_no}_{self.dataset}.pkl'  # changed : to -
    ...
    def _construct_exemplar_set(self, images, m):
        ...
        self.exemplar_transforms.append(self.transform) #new
        self.exemplar_classify_transforms.append(self.classify_transform) #new
    ...
    def Image_transform(self, images, transform): 
        ...
        for index in range(1, len(images)):
            data = torch.cat((data, transform(Image.fromarray(images[index])).unsqueeze(0)), dim=0)    #added convert since some exemplars were coming out as grayscale.
    ...
    def compute_class_mean(self, images, transform):
        if self.dataset == 'CIFAR10' or self.dataset == 'MNIST' :
            #computing class mean in batches to avoid OOM for CIFAR10
            if self.dataset == 'CIFAR10':
                batch_size = 1500
            elif self.dataset == 'MNIST':
                batch_size = 3500
            ...
            for i in range(0, len(images), batch_size):
                ...
                x = self.Image_transform(batch_images, transform).to(device)
                with torch.no_grad():
                    ...
                    all_features.append(feature_extractor_output)
                    del x
                    torch.cuda.empty_cache()
            feature_extractor_output = np.concatenate(all_features, axis=0) #concatenate features from all batches
            class_mean = np.mean(feature_extractor_output, axis=0)
            return class_mean, feature_extractor_output
        
        elif self.dataset == 'CIFAR100': #in the case of CIFAR100, the number of images per class is small, so we can compute class mean in one go without worrying about OOM
    ...
    def compute_exemplar_class_mean(self):
        ...
        for index in range(len(self.exemplar_set)):
            ...
            trans = self.exemplar_transforms[index] #new
            classify_trans = self.exemplar_classify_transforms[index] #new
            #exemplar=self.train_dataset.get_image_class(index)
            class_mean, _ = self.compute_class_mean(exemplar, trans)
            class_mean_,_=self.compute_class_mean(exemplar, classify_trans)
